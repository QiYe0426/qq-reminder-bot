...
- 日报工具化：
  - 新增 `generate_daily_report` Agent 工具，管理员可在群里自然询问“昨日总结”“昨天日报”“生成某日总结”。
  - 工具复用现有日报生成链路和消息采集数据；默认日期为昨天，指定日期支持“今天 / 昨天 / 前天 / YYYY-MM-DD”。
  - 未开启消息采集或日报功能的群会返回明确原因，不会静默失败。
- 画像和群状态工具化：
  - 新增 `get_group_status` Agent 工具，管理员可询问当前群功能开关、采集统计、群画像、画像记录人数和 Agent 工具权限。
  - 新增 `get_group_profile` Agent 工具，管理员可自然询问当前群画像。
  - 新增 `get_member_profile` Agent 工具，管理员可按 QQ 号或昵称关键词查看某个群友画像；对象不唯一时会返回候选并要求进一步确认。
  - 群画像继续接入 AI 上下文，同时作为显式 Agent 工具可被主动调用。
- Agent 工具权限：
  - 新增 `plugins/agent_tool_access.py`，用 `group_agent_tool_settings` 记录“某群是否允许某个 Agent 工具”。
  - Agent 每次下发工具列表前会按当前群、当前用户权限、功能依赖和工具开关动态过滤。
  - 即使模型硬调用已关闭或无权限的工具，执行前也会再次拦截。
  - 管理类工具需要管理员权限；普通群友不能通过 AI 对话读取日报、群状态或他人画像。
- 控制台增强：
  - 群管理页新增“Agent 可用能力”面板，显示当前系统注册的工具、类别、依赖和权限要求。
  - 群管理页新增“本群 Agent 工具权限”面板，可逐群开关联网搜索、提醒、STS2 知识库、群上下文、日报、画像、群状态等工具。
  - `respond` 作为 Agent 核心回复能力固定可用，不允许关闭。

### 测试

- 新增 Agent 工具权限测试，覆盖默认开启、逐群关闭、重新开启和管理员权限拦截。
- 更新 Agent 工具注册测试，覆盖日报、群状态、群画像和群友画像工具。
...
- 新增 `plugins/knowledge_service.py` 和 `search_sts2_knowledge` Agent 工具：
  - STS2 知识库采用混合方案：普通 AI 对话仍会轻量预加载少量相关知识，Agent 也可以按需主动检索本地 STS2 卡牌、遗物、角色、敌人、Boss、事件、关键词、机制和攻略。
  - 中文检索增强：标题或关键词被包含在完整问题中时也会命中，适配“药水腰带怎么样”这类自然问法。
  - 工具只检索 `STS2` 分类，避免把非 STS2 手写知识混进 STS2 回答。
- 新增 `plugins/group_context_service.py` 和 `get_group_context` Agent 工具：
  - 开启 `消息采集` 的群，工具读取本群归档消息，并支持关键词筛选。
  - 未开启 `消息采集` 的群，工具只读取进程内最近临时上下文，保持 AI 对话独立可用。
  - 原有轻量群上下文预加载也改为复用同一服务，减少两套逻辑分叉。
- 提醒 Agent 工具收束：
  - AI 对话里的自然语言提醒跟随 `AI 对话` 开关常开，不再要求单独开启传统 `提醒` 功能。
  - 明确的提醒请求会先走本地解析和创建，减少不必要的大模型等待。
  - 群提醒到点时会 `@` 创建提醒的人；提醒内容里的 CQ 码会被转义，避免用户输入变成 QQ 指令。
- 提醒别人：
  - 群聊自然语言提醒支持 `@猎bot 10分钟后提醒小明喝水` 和 `@猎bot 10分钟后提醒 @小明 喝水`。
  - 直接 @某人时，到点提醒会 @被提醒人；按昵称/群名片识别时，如果匹配不确定，会询问“你想提醒的是 xxx 对吗”。
  - 发起人在 1 分钟内可以不 @bot 直接回复 `对` / `不对`，也可以直接 @某人；确认、取消或改用 @某人后，临时确认状态立即失效。
  - 创建成功反馈会用纯文本说明“提醒谁：提醒内容”，不会在创建阶段 @ 被提醒人；到点真正提醒时才会 @ 被提醒人。
- 108 攻略差异导入：
  - 新增 `knowledge_sources/sts2_guides/main_108.tex` 作为 108 相对 107 的差异源，不会完整覆盖或重复导入 107 攻略。
  - `sts_knowledge_seed.py` 会按 LaTeX 章节对比 `main_1.tex` 和 `main_108.tex`，只把正文变化章节导入 `STS2/guide/version-diff`。
  - 当前收到的 108 文件与 107 文件只有 BOM/空格等排版级差异，正文归一化后没有可导入的攻略内容差异；机制已就绪，待拿到真正正文变化的 108 文件后会自动生成差异条目。
- Agent 系统说明更新：
  - STS2 问题优先调用本地知识库工具。
  - 需要回忆、查找、总结当前群最近聊天时调用群上下文工具。

### 测试

- 新增知识库服务、群上下文服务和 Agent 工具注册测试。
- 本地测试覆盖提升到 45 项，包含提醒、提醒别人、报时、审批脱敏、功能依赖、知识库工具、群上下文工具、108 攻略差异导入和插件配置。

## v2.0.1 - Codex QQ 远程审批
...
- 重整群功能依赖：`消息采集` 是基础数据层，`日报` 和 `智能陪伴` 依赖消息采集，`AI 对话` 独立运行；消息采集开启时 AI 可读取最近采集消息，未开启时只读取进程内最近约 20 条临时上下文。
