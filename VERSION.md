...
- 新增 `plugins/semantic_graph.py`：
  - 基于已采集群消息抽取发言人、话题/关键词、媒体素材节点。
  - 生成“提到”“相关”“接续讨论”等关系边。
  - 语义图保存到 `data/semantic_graph.db`，支持按最近消息、指定日期和关键词生成。
- 新增 `plugins/semantic_graph_visual.py`：
  - 将语义图渲染成 PNG 关系图，输出到 `data/semantic_graphs/`。
  - 可视化区分发言人、话题节点和主要关系强度。
- 新增 Agent 工具：
  - `build_semantic_graph`：管理员生成或刷新指定群语义图。
  - `get_semantic_graph`：管理员读取指定群最新语义图摘要。
  - `render_semantic_graph`：管理员生成语义图 PNG，并尽量发送到当前 QQ 会话。
- 控制台 Agent 能力和本群工具权限面板会显示语义图相关工具，可按群关闭或开启。
- Agent 系统提示词增加语义图调用规则：管理员询问话题关系、语义图、话题图谱、群知识结构或要求“画出来”时优先调用语义图工具。

### 测试

- 新增语义图构建、落库、读取、可视化渲染和 Agent 工具调用测试。
...
- 修复管理员私聊索要日报时，如果服务重启或发布打断旧生成流程，可能只看到“开始生成”但没有后续回复的问题。
- 私聊 `生成日报 ...` 现在会先检查 `data/reports/` 中同群同日期的已生成 Markdown、PNG 长图和 PDF：
  - 找到成品时直接发送长图/PDF，并提示“复用已生成文件”。
  - 文件名中的群名变更也可识别，只要日期和群号一致即可匹配。
  - 找不到成品时才重新调用 AI 生成。
- 自动日报、`测试日报 ...` 和管理员自然语言日报 Agent 工具也会优先复用已生成文件，避免重复生成和长时间卡在 running。
- 管理员自然语言查询日报时，如果本地已有 Markdown 成品，会直接读取给 Agent 回复，不再重新进入 AI 摘要生成链路。

### 测试

- 本地虚拟环境执行 `python -m pytest`：64 passed。
...
- 排查 2026-07-08 04:00 自动日报：群 `1072225932` 在最终 AI 合成阶段超过 90 秒，触发 `asyncio.exceptions.TimeoutError`，导致该群日报失败；群 `722290838` 正常生成。
- 日报 AI 调用默认超时从 90 秒提高到 180 秒。
- 新增 `SUMMARY_RETRY_ATTEMPTS` 和 `SUMMARY_RETRY_BACKOFF_SECONDS`：
  - AI 摘要或合成接口偶发慢响应时会自动重试。
  - 默认重试 2 次，间隔 3 秒。
- 分块摘要失败时不再中断整份日报，会写入自动摘录兜底内容。
- 最终 AI 合成失败时不再导致日报整体失败，会用分块摘要生成“兜底版日报”，并在日报中明确标注生成提示和失败原因。

### 测试

- 新增日报兜底测试，覆盖分块摘要失败摘录和最终合成失败兜底版日报格式。
...
- 提醒别人时，需要确认提醒对象的消息增加明显标题 `【提醒确认】`。
- 对象已确认但还缺提醒时间时，消息增加标题 `【提醒时间】`。
- 确认/等待消息会明确提示 `1 分钟内有效`，避免看起来像普通闲聊。

### 测试

- 新增提醒提示文案测试，锁定确认标题和时间等待标题格式。
...
- 日报新增同版式长图和 PDF：
  - 每次生成日报都会保存 Markdown、PNG 长图和 PDF 到 `data/reports/`。
  - PNG 长图采用“猎宝日报”绿色卡片式版式，包含概览、今日重点、分话题总结和话题语义图。
  - PDF 不再使用纯文字排版，而是嵌入同一张长图，保证图片和 PDF 视觉一致，方便长期留档。
- 手动私聊 `生成日报 ...` 会先发送长图，再发送 PDF 文件；自动日报会私聊管理员发送长图，并提示 PDF 留档文件名。
- 长图发送失败时不会吞掉结果，会继续尝试发送 PDF 或返回明确失败原因。

### 测试

- 新增视觉日报渲染测试，覆盖 PNG 长图尺寸和 PDF 单页生成。
...
- 修复缺少有效时间的提醒请求会被大模型口头追问、但代码没有进入等待状态的问题：
  - 例如 `@猎bot 提醒小拉现在抱抱小匣草草小匣` 会保存提醒对象和内容，并等待发起人在 1 分钟内补 `一分钟后`、`明早9点` 等时间。
  - 如果对象不确定，猎宝会保存这个待办，等用户回答 `对/不对` 或直接 `@某人`，不会只是随口问一句。
- 修复 `一分钟后提醒小拉现在抱抱...` 这类对象短称未精确命中时会退回成提醒发起人自己的问题：
  - 现在会使用受控模糊匹配；对象不够确定时先问确认，不会直接创建自提醒。

### 测试

- 新增群昵称短称模糊识别测试，覆盖 `小拉现在...` 对应群名片 `小拉草草小匣` 时先进入对象确认。
...
- 修复 `@猎bot 提醒匣子5分钟后睡觉` 这类“提醒对象在时间前”的语序没有进入本地提醒解析的问题。
- 新增 `提醒5分钟后睡觉`、`提醒我5分钟后睡觉`、`提醒某人5分钟后睡觉` 三类解析测试。
- 修复后，“提醒某人5分钟后做什么”会先规整成内部内容 `提醒某人做什么`，再走群成员昵称/群名片匹配；如果能明确匹配，会直接创建到点 @ 对方的提醒。
...
- 修复群聊提醒别人时的跨消息指代：
  - 支持 `12:00提醒匣子睡觉` 这类时间后无空格的写法进入本地快速解析。
  - 成功创建“提醒别人”后，会按“同一群 + 同一发起人”短期记住最近提醒对象。
  - 后续 `再在凌晨1点提醒她睡觉`、`提醒 ta 喝水` 等说法会复用最近提醒对象；确认对象的 1 分钟临时状态仍然在确认、取消或 @某人后立即失效。
- 日报 Agent 工具增加生成反馈：
  - 开始生成时会主动发送“开始生成”提示。
  - 功能未开启、AI 生成异常等失败会写入 `daily_report_runs` 并返回明确错误原因。
  - 群状态工具会显示最近日报运行状态，方便排查“没回复 / 卡住 / 失败”。

### 新增

- 管理员私聊管理群能力：
  - `get_group_status`、`generate_daily_report`、`get_group_profile`、`get_member_profile` 支持在私聊里带 `group_id` 操作指定群。
  - 新增 `set_group_features` Agent 工具，管理员可私聊自然语言开启、关闭或调整指定群功能。
  - 权限仍由 `BOT_ADMIN_USER_IDS` 控制；普通用户不能通过私聊读取或调整群管理能力。

### 测试

- 新增提醒无空格时间解析、跨消息指代文本清理、管理员私聊工具权限和群功能调整测试。
